...
from modelos import Movimiento, MovimientoSchema,Categoria, db
...
class VistaMovimiento(Resource):
    ...
    def put(self, id_movimiento):
        ...
        if not self.es_posible_eliminar_actualizar_movimiento(movimiento):
            return {'mensaje': 'No es posible actualizar este movimiento esta relacionado con una propiedad'}, 400
    ...
    def es_posible_eliminar_actualizar_movimiento(self, movimiento):
        if movimiento.categoria == Categoria.INGRESOS_POR_RESERVAS:
            return False
