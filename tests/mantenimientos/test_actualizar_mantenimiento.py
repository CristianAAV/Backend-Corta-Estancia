...
from modelos import Usuario, Propiedad, Mantenimiento, MantenimientoSchema, Banco, TipoMantenimiento, Estado, db
from datetime import datetime, timedelta

from modelos.modelos import TipoUsuario


class TestActualizarMantenimiento:
    ...
    def setup_method(self):
        ...
        self.nuevos_datos_mantenimiento = {
    ...
    def actuar(self, client, id_mantenimiento, nuevos_datos_mantenimiento=None, token=None):
        nuevos_datos_mantenimiento = nuevos_datos_mantenimiento or self.nuevos_datos_mantenimiento
        headers = {'Content-Type': 'application/json'}        
        print(self.mantenimiento_aires.fecha)
        if token:
            headers.update({'Authorization': f'Bearer {token}'})
        self.respuesta = client.put(f'/mantenimientos/{id_mantenimiento}', data=json.dumps(nuevos_datos_mantenimiento), headers=headers)
        self.respuesta_json = self.respuesta.json

    def test_retorna_200_mantenimiento_se_puede_actualizar_pertenece_a_propiedad_usuario_token(self, client, mock_datetime_now):
    ...
    def test_retorna_200_actualizar_mantenimiento_piscina(self, client, mock_datetime_now):
        mock_datetime_now(self.mantenimiento_piscina.fecha)
        token_usuario_1 = create_access_token(identity=self.usuario_1.id)
        self.actuar(client, self.mantenimiento_piscina.id, token=token_usuario_1)
        assert self.respuesta.status_code == 200

    def test_retorna_200_actualizar_mantenimiento_aires(self, client, mock_datetime_now):
        mock_datetime_now(self.mantenimiento_aires.fecha)
        token_usuario_1 = create_access_token(identity=self.usuario_1.id)
        self.actuar(client, self.mantenimiento_aires.id, token=token_usuario_1)
        assert self.respuesta.status_code == 200

    def test_retorna_200_actualizar_mantenimiento_mes_anterior(self, client, mock_datetime_now):
